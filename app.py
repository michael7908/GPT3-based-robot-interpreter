...
aws_default_region = "the format should be us-west-?"
aws_default_s3_bucket = "your_bucket_name"
...
def get_gpt_answer(prompt):
    ...
    print(f"[INFO] ChatGPT answer: {answer}")
    return answer
...
def speak_script(output_file):
    ...
    if os.name == "nt":
        # for windows
        subprocess.run(["powershell.exe", "-Command", "Add-Type -AssemblyName System.Speech; (New-Object System.Speech.Synthesis.SpeechSynthesizer).Speak('"+output_file+"')"], check=True)
    else:
        # for MacOS
        try:
            subprocess.run(["afplay", output_file], check=True, text=True)

        except:
            print("Error: failed to play the audio file")
...
def get_speaker(language_detected):
    ...
    if language_detected == "en-US":
        ...
        voice = "Kajal"
    elif language_detected == "cmn-CN":
        voice = "Zhiyu"
    elif language_detected == "zh-CN":
        voice = "Zhiyu"
    elif language_detected == "es-MX":
        voice = "Mia"
    elif language_detected == "en-US":
        voice = "Lupe"
    else:
        voice = "Joanna"
